...
| Advanced | Human-AI Futures | **[VeriLoop](cothink/)** (evolved from CoThink) | Evidence-in-the-loop AI workspace: claims decomposed, verified claim-by-claim by a second model, contradictions surfaced, decisions engine-gated on verification — **44/44 tests** incl. the confident-but-wrong demo scenario |
...
├── cothink/        # Tier 3 — VeriLoop: evidence-in-the-loop AI workspace (Node server)
...
│   ├── lib/claims.cjs           # deterministic claim decomposition
│   ├── lib/verifier.cjs         # evidence grading + contradiction detection
...
cd cothink    && npm test        # 44 tests: claims, verifier, engine, API, attacks
```
...
- **Evidence is the gate, not a nicety.** VeriLoop's decision engine
  refuses any state transition that is not an explicit human decision —
  enforced in tests, not in a code-of-conduct page.
- **Every claim is tested.** No golden-doc-only projects: 71 automated tests
